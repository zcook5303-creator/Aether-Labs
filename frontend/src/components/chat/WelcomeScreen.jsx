    ...
    <div className={`flex-1 flex flex-col items-center justify-center px-4 py-8 ${theme === 'light' ? 'bg-zinc-100 text-zinc-900' : ''}`} data-testid="welcome-screen">
      <div className={`max-w-md w-full text-center space-y-6 ${theme === 'light' ? 'bg-white/95 rounded-3xl shadow-xl px-6 py-8' : ''}`}>
        ...
        <div className="space-y-3 pt-2">
          ...
          <button onClick={() => setModal('review')} className={`w-full flex items-center justify-center gap-3 px-6 py-3 rounded-xl border transition-colors ${theme === 'light' ? 'border-zinc-200 text-zinc-700 hover:bg-zinc-100' : 'border-white/10 text-white/70 hover:bg-white/5 hover:text-white'}`} data-testid="review-button">
